...
saveRDS(Baboon_vigilance_stats_both_nocontrol, "data_derived/Baboon_vigilance_stats_both_nocontrol.rds")
...
#generate model set
Flight_models_both <- dredge(Flight_global_model_both)

# Model averaging based on AIC
Flight_model_avg_both <- model.avg(Flight_models_both)

# Get model-averaged results
print(Flight_model_avg_both)

#extract model weights
Flight_model_weights <- sw(Flight_model_avg_both)
print(Flight_model_weights)

# 95% confidence intervals for averaged parameters
confint(Flight_model_avg_both, level = 0.95)

...
source("scripts/4. Model_predictions_fixed.R")
source("scripts/5. Raw_data_figures.R")
...
combined_flight_pred_plot <- ggplot() +
  # Observed bar plot (no error bars)
  geom_bar(
    data = flight_pred,
    aes(x = predator_cue, y = flight, fill = "Observed"),
    stat = "identity",
    position = position_dodge(width = 0.9),
...
# Flight by year
combined_flight_year_plot <- ggplot() +
  # Observed bar plot (no error bars)
  geom_bar(
    data = flight_year,
    aes(x = year, y = flight, fill = "Observed"),
    stat = "identity",
    width = 0.6,
    color = "black", alpha = 0.8
  ) +
...
# Flight by habitat
combined_flight_habitat_plot <- ggplot() +
  # Observed bar plot (no error bars)
  geom_bar(
    data = flight_habitat,
    aes(x = Habitat, y = flight, fill = "Observed"),
    stat = "identity",
    width = 0.6,
    color = "black", alpha = 0.8
  ) +
...
combined_flight_prey_plot <- ggplot() +
  # Observed: bars only (no error bars)
  geom_bar(
    data = Baboon_flight_graph_both,
    aes(x = age_sex_class, y = flight, fill = "Observed"),
    stat = "identity",
    width = 0.6,
    alpha = 0.8,
    color = "black"

#Combining AVERAGED model predictions and box/whisker plots 
...
combined_avg_vigilance_predator_plot <- ggplot() +
  ...
  geom_errorbar(
    data = vigilance_avg_pred_only,
    aes(x = predator_cue, ymin = conf.low, ymax = conf.high, color = "Predicted"),
    width = 0.2,
    position = position_dodge(width = 0.9)
  ) +
  labs(
...
#vigilance by year
combined_avg_vigilance_year_plot <- ggplot() +
  # Boxplot with jittered raw data
  geom_boxplot(
    data = Baboon_vigilance_stats_both,
  ...
  geom_errorbar(
    data = vigilance_avg_year_only,
    aes(x = year, ymin = conf.low, ymax = conf.high, color = "Predicted"),
    width = 0.2,
    position = position_dodge(width = 0.9)
  ) +
  labs(
...
combined_avg_vigilance_habitat_plot <- ggplot() +
  # Boxplot with jittered raw data
  geom_boxplot(
    data = Baboon_vigilance_stats_both,
  ...
  # Model predictions (points + error bars)
  geom_point(
    data = vigilance_avg_habitat_only,
  ...
  geom_errorbar(
    data = vigilance_avg_habitat_only,
    aes(x = habitat, ymin = conf.low, ymax = conf.high, color = "Predicted"),
    width = 0.2,
    position = position_dodge(width = 0.9)
  ) +
  labs(
...
combined_avg_vigilance_prey_plot <- ggplot() +
  # Boxplot with jittered raw data
  geom_boxplot(
    data = Baboon_vigilance_stats_both,
  ...
  geom_errorbar(
    data = vigilance_avg_prey_only,
    aes(x = age_sex_class, ymin = conf.low, ymax = conf.high, color = "Predicted"),
    width = 0.2,
    position = position_dodge(width = 0.9)
  ) +
  labs(
...
combined_avg_frequency_pred_plot <- ggplot() +
...
# Flight by year
combined_avg_frequency_year_plot <- ggplot() +
  # Observed bar plot (no error bars)
  geom_bar(
...
# Flight by habitat
combined_avg_frequency_habitat_plot <- ggplot() +
  # Observed bar plot (no error bars)
  geom_bar(
  ...
  # Predicted values with CI
  geom_point(
    data = frequency_avg_habitat_only,
    aes(x = habitat, y = predicted, color = "Predicted"),
    size = 3,
    position = position_dodge(width = 0.6)
  ) +
...
combined_avg_frequency_prey_plot <- ggplot() +
